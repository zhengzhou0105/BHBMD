...
model{
  ...
  sigma_c ~ exponential(prior_sigma_c);
  // print("sigma_c = ",target());
  ...
  for(s in 1:S){
    ...
    seg_fdL = to_array_1d(log(a[s] + (b[s] * to_vector(pow_vec(seg_doseZ,g[s]))) ./ (c[s]^g[s] + to_vector(pow_vec(seg_doseZ,g[s]))) ));  // use fdL instead of log(fd)
    // seg_fdL = to_array_1d(log(a[s] + (b[s] * to_vector(pow_vec(seg_dose,g[s]))) ./ (c[s]^g[s] + to_vector(pow_vec(seg_dose,g[s]))) ));  // use fdL instead of log(fd)
...
generated quantities{
  ...
  for(n in 1:N){
    fd[n] = a[Index[n]] + (b[Index[n]] * doseZ[n]^g[Index[n]])/(c[Index[n]]^g[Index[n]]+doseZ[n]^g[Index[n]]);                          // simulate fd for double check
    // fd[n] = a[Index[n]] + (b[Index[n]] * dose[n]^g[Index[n]])/(c[Index[n]]^g[Index[n]]+dose[n]^g[Index[n]]);                          // simulate fd for double check
