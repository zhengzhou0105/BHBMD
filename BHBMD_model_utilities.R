...
get_metaStanfit <- function(modelname,dataName,DataList,
  ...
  a_bound <- log(ybound[1] + 2 * ybound[2])

  source("BHBMD_priors.R",local = T)
...
fit_allmodels <- function(DSName,DataList){
  ...
  modelname <- "Lognormal_Summary_Linear_meta_2"
  assign(paste0("stanfit_",modelname,"_",dataName),
         get_metaStanfit(modelname = modelname,dataName = dataName,DataList = DataList))
  modelname <- "Lognormal_Summary_Power_meta_3"
  assign(paste0("stanfit_",modelname,"_",dataName),
         get_metaStanfit(modelname = modelname,dataName = dataName,DataList = DataList))
  modelname <- "Lognormal_Summary_Hill_meta_4"
  assign(paste0("stanfit_",modelname,"_",dataName),
         get_metaStanfit(modelname = modelname,dataName = dataName,DataList = DataList))
  modelname <- "Lognormal_Summary_Expo5_meta_4"
  assign(paste0("stanfit_",modelname,"_",dataName),
         get_metaStanfit(modelname = modelname,dataName = dataName,DataList = DataList))
  ...
  assign(
    ...
    list(
      Linear = eval(parse(text = paste0("stanfit_Lognormal_Summary_Linear_meta_2_",dataName))),
      Hill = eval(parse(text = paste0("stanfit_Lognormal_Summary_Hill_meta_4_",dataName))),
      Power = eval(parse(text = paste0("stanfit_Lognormal_Summary_Power_meta_3_",dataName))),
      Expo5 = eval(parse(text = paste0("stanfit_Lognormal_Summary_Expo5_meta_4_",dataName)))
    )
  )
...
getwts <- function(fitList){
  Nmod <- length(fitList)
  Npars <- as.numeric(unlist(lapply(fitList,getNpars)))
