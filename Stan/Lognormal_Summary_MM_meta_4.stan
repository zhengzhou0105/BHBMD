...
model{
  ...
  for(s in 1:S){
    ...
    seg_fdL = to_array_1d(log(a[s]+b[s]*to_vector(seg_doseZ) ./ (c[s]+to_vector(seg_doseZ))));  // use fdL instead of log(fd)
    // seg_fdL = to_array_1d(log(a[s]+b[s]*to_vector(seg_dose) ./ (c[s]+to_vector(seg_dose))));     // use fdL instead of log(fd)
  
    target += Normal_Summary_lpdf(seg_ymeanL | seg_ysdL, seg_Nsub, seg_fdL, sigma);
    // print("target LL= ",target());
...
generated quantities{
  ...
  for(n in 1:N){
    fd[n] = a[Index[n]] + b[Index[n]] * doseZ[n] /(c[Index[n]]+doseZ[n]);                          // simulate fd for double check
    // fd[n] = a[Index[n]] + b[Index[n]] * dose[n] /(c[Index[n]]+dose[n]);
