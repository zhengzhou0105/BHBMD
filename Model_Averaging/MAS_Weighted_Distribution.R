...
for(b in 1:batch){
  for(i in 1:iter){
    ...
    # weighted_batch_temp <- vector("list",Napproach)
    # for(m in 1:Napproach){
    #   colstart <- Nobs*(m-1)+1
    #   colend <- Nobs*m
    #   weighted_batch_temp[[m]] <- weighted_temp[,colstart:colend]
    #   colnames(weighted_batch_temp[[m]]) <- Xnames
    # }
    weighted_batch[[index]] <- weighted_temp
  }
}
...
for(b in 1:batch){
  for(i in 1:iter){
    index <- batchsize * (b-1)+i
    Data_temp <- read.csv(file = paste0("DataList_batch_",DSName,"_batch_",b,"_",i,".csv"))
...
svg(filename = "Robustness of Averaging Performance.svg",
    width = 12,height = 12)
print(plot)
dev.off()
