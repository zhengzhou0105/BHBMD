...
get_BHBMD_posterior <- function(
    stanfit,level,
    model = "all", Index = "all"){
  
  if(level == "overarching"){
    output <- get_overarching(stanfit,model)
  } else if(level == "specific"){
    output <- get_specific(stanfit,model,Index)
  }
  
  return(output)
}

get_overarching <- function(stanfit,model){
  # one model or all four
  if(model == "all"){
    output <- list(
      Linear = get_Linear_overarching(stanfit = stanfit$Linear),
      Power = get_Power_overarching(stanfit = stanfit$Power),
      Hill = get_Hill_overarching(stanfit = stanfit$Hill),
      Expo5 = get_Expo5_overarching(stanfit = stanfit$Expo5)
    )
  } else {
    stanfit <- stanfit[[model]]
    output <- get(paste0("get_",model,"_overarching"))(stanfit = stanfit)
  }
  
  return(output)
}

get_specific <- function(stanfit,model,Index){
  # one model or all four
  if(model == "all"){
    output <- list(
      Linear = get_Linear_specific(stanfit$Linear,Index = Index),
      Power = get_Power_specific(stanfit$Power,Index = Index),
      Hill = get_Hill_specific(stanfit$Hill,Index = Index),
      Expo5 = get_Expo5_specific(stanfit$Expo5,Index = Index)
    )
  } else {
    stanfit <- stanfit[[model]]
    output <- get(paste0("get_",model,"_specific"))(stanfit,Index = Index)
  }
  
  return(output)
}

get_Linear_overarching <- function(stanfit){
...
get_Power_overarching <- function(stanfit){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  
  a_bound <- 0
  source(
    file = paste0(fileDir,"BHBMD_priors.R"),local = T
  )
  a <- rlnorm(iterEff,df_posteriors$mu_a,df_posteriors$sigma_a)
  b <- extraDistr::rhcauchy(iterEff,df_posteriors$scale_b)
  # g <- rlnorm(iterEff,df_posteriors$mu_g,df_posteriors$sigma_g)
  g <- rlnormTrunc(iterEff,df_posteriors$mu_g,df_posteriors$sigma_g,
...
get_MM_overarching <- function(stanfit){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
...
get_Hill_overarching <- function(stanfit){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  ...
  c <- rlnorm(iterEff,df_posteriors$mu_c,df_posteriors$sigma_c)
  g <- rlnormTrunc(iterEff,df_posteriors$mu_g,df_posteriors$sigma_g,
                   min = prior_Lognormal_Summary_Hill_meta_5$g_constr[1], 
                   max = prior_Lognormal_Summary_Hill_meta_5$g_constr[2])
...
get_Expo5_overarching <- function(stanfit){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  
  a <- rlnorm(iterEff,df_posteriors$mu_a,df_posteriors$sigma_a)
  b <- extraDistr::rhcauchy(iterEff,df_posteriors$scale_b)
  d <- rlnormTrunc(iterEff,df_posteriors$mu_d,df_posteriors$sigma_d,
                   max = 18)
  c <- vector("numeric",iterEff)
  for(i in 1:iterEff){
    c[i] <- extraDistr::rhcauchy(1,df_posteriors$scale_c)
    while (c[i] <= 1) {
      c[i] <- extraDistr::rhcauchy(1,df_posteriors$scale_c)
    }
  }
...
get_Linear_specific <- function(stanfit,Index){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  
  ## check if index is within range
  NIndex <- length(Index)
  NStudy <- sum(!is.na(stringr::str_match(colnames(df_posteriors),"a.\\d.")))
  if(is.numeric(Index) & max(Index) > NStudy){
    stop("Specified study index not within range")
  }
  if(NIndex == 1 & Index[1] == "all") {NIndex <- NStudy}
  
  ## get output
  if(is.numeric(Index) & NIndex == 1){
    ## output one study
    output <- data.frame(
      a = df_posteriors[,paste0("a[",Index,"]")],
      b = df_posteriors[,paste0("b[",Index,"]")]
    )    
  } else if (is.numeric(Index) & NIndex > 1){
    ## output multiple studies
    output <- vector("list",NIndex)
    names(output) <- paste0("study",Index)
    for(i in 1:NIndex){
      output[[i]] <- data.frame(
        a = df_posteriors[,paste0("a[",i,"]")],
        b = df_posteriors[,paste0("b[",i,"]")]
      )
    }
  } else {
    ## all studies
    output <- vector("list",NIndex)
    names(output) <- paste0("study",1:NStudy)
    for(i in 1:NIndex){
      output[[i]] <- data.frame(
        a = df_posteriors[,paste0("a[",i,"]")],
        b = df_posteriors[,paste0("b[",i,"]")]
...
get_Power_specific <- function(stanfit,Index){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  ...
  if(is.numeric(Index) & NIndex == 1){
    ...
    output <- data.frame(
      a = df_posteriors[,paste0("a[",Index,"]")],
      b = df_posteriors[,paste0("b[",Index,"]")],
      g = df_posteriors[,paste0("g[",Index,"]")]
    )    
  } else if (is.numeric(Index) & NIndex > 1){
    ## output multiple studies
    output <- vector("list",NIndex)
...
get_MM_specific <- function(stanfit,Index){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  ...
  if(is.numeric(Index) & NIndex == 1){
    ...
    output <- data.frame(
      a = df_posteriors[,paste0("a[",Index,"]")],
      b = df_posteriors[,paste0("b[",Index,"]")],
      c = df_posteriors[,paste0("c[",Index,"]")]
    )    
  } else if (is.numeric(Index) & NIndex > 1){
    ## output multiple studies
    output <- vector("list",NIndex)
...
get_Hill_specific <- function(stanfit,Index){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
...
get_Expo5_specific <- function(stanfit,Index){
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  ...
  if(is.numeric(Index) & NIndex == 1){
    ...
    output <- data.frame(
      ...
      b = df_posteriors[,paste0("b[",Index,"]")],
      c = df_posteriors[,paste0("c[",Index,"]")],
      d = df_posteriors[,paste0("d[",Index,"]")]
    )    
  } else if (is.numeric(Index) & NIndex > 1){
    ## output multiple studies
    output <- vector("list",NIndex)
...
get_Weighted <- function(fitList,df_wts,input,
                         level,Index){
  ModelList <- c("Linear","Hill","Power","Expo5")
  Nmodel <- length(ModelList)
  ApproList <- c("AIC","BIC","WAIC","LPML","Stacking","PseudoBMA","PseudoBMA_BB")
  Nappr <- length(ApproList)
  Nobs <- length(input)
  iterEff <- nrow(as.data.frame(fitList$Linear))
  
  ## get fd by model
  
  ## Linear
  Linear_pars <- get_BHBMD_posterior(
    stanfit = fitList,level = level, Index = Index, model ="Linear"
  )
  # df_posteriors <- as.data.frame(fitList$Linear)
  # a <- rlnorm(iterEff,df_posteriors$mu_a,df_posteriors$sigma_a)
  # b <- rhcauchy(iterEff,df_posteriors$scale_b)
  fd_Linear <- matrix(NA, nrow = iterEff,ncol = Nobs)
  for(i in 1:iterEff){
    # fd_Linear[i,] <- get_Linear(input,a[i],b[i])
    fd_Linear[i,] <- get_Linear(
      x = input,
      a = Linear_pars$a[i],
      b = Linear_pars$b[i]
    )
  ...
  ## Power
  Power_pars <- get_BHBMD_posterior(
    stanfit = fitList,level = level,Index = Index,model = "Power"
  )
  # df_posteriors <- as.data.frame(fitList$Power)
  # a <- rlnorm(iterEff,df_posteriors$mu_a,df_posteriors$sigma_a)
  # b <- extraDistr::rhcauchy(iterEff,df_posteriors$scale_b)
  # g <- rlnormTrunc(iterEff,df_posteriors$mu_g,df_posteriors$sigma_g,
  #                  min = 1, max = 18)
  fd_Power <- matrix(NA, nrow = iterEff,ncol = Nobs)
  for(i in 1:iterEff){
    # fd_Power[i,] <- get_Power(input,a[i],b[i],g[i])
    fd_Power[i,] <- get_Power(
      x = input,
      a = Power_pars$a[i],
      b = Power_pars$b[i],
      g = Power_pars$g[i]
  ...
  ## Hill
  Hill_pars <- get_BHBMD_posterior(
    stanfit = fitList,level = level,Index = Index, model = "Hill"
  )
  # df_posteriors <- as.data.frame(fitList$Hill)
  # a <- rlnorm(iterEff,df_posteriors$mu_a,df_posteriors$sigma_a)
  # b <- extraDistr::rhcauchy(iterEff,df_posteriors$scale_b)
  # c <- rlnorm(iterEff,df_posteriors$mu_c,df_posteriors$sigma_c)
  # g <- rlnormTrunc(iterEff,df_posteriors$mu_g,df_posteriors$sigma_g,
  #                  min = 1, max = 18)
  fd_Hill <- matrix(NA, nrow = iterEff,ncol = Nobs)
  for(i in 1:iterEff){
    # fd_Hill[i,] <- get_Hill(input,a = a[i], b= b[i],c = c[i],g = g[i])
    fd_Hill[i,] <- get_Hill(
      x = input,
      a = Hill_pars$a[i],
      b = Hill_pars$b[i],
  ...
  ## Expo5
  # df_posteriors <- as.data.frame(fitList$Expo5)
  # a <- rlnorm(iterEff,df_posteriors$mu_a,df_posteriors$sigma_a)
  # b <- extraDistr::rhcauchy(iterEff,df_posteriors$scale_b)
  ...
  #   }
  # }
  Expo5_pars <- get_BHBMD_posterior(
    stanfit = fitList,level = level, Index = Index, model = "Expo5"
  )
  
  fd_Expo5 <- matrix(NA, nrow = iterEff,ncol = Nobs)
  for(i in 1:iterEff){
    # fd_Expo5[i,] <- get_Expo5(input,a = a[i], b= b[i],c = c[i],d = d[i])
    fd_Expo5[i,] <- get_Expo5(
      x = input,
      a = Expo5_pars$a[i],
      b = Expo5_pars$b[i],
      c = Expo5_pars$c[i],
  ...
  fd_list <- list(
    Linear = fd_Linear,Hill = fd_Hill,Power = fd_Power,Expo5 = fd_Expo5
  )
  # rm(list = c("fd_Linear","fd_Power","fd_Hill","fd_Expo5"))
  
  fd_weighted <- vector("list",Nappr)
  for(a in 1:Nappr){
    fd_weighted[[a]] <- matrix(0,nrow = iterEff,ncol = Nobs)
    for(m in 1:Nmodel){
      fd_weighted[[a]] <- fd_weighted[[a]] + fd_list[[m]] * df_wts[m,a]
    }
  }
...
plot_overarching_Power <- function(stanfit,DataList,
                                   ...
                                   xup = NULL, yup = NULL,
                                   savetag = "curve"){
  # source(file = paste0(fileDir,"BHBMD_priors.R"),local = T)
  # posteriors
  df_posteriors <- as.data.frame(stanfit)
  iterEff <- nrow(df_posteriors)
  NStudy <- length(unique(DataList$Index))          # number of studies
